...
import { utils, uuid } from 'vscode-common';
...
export class GitHubOAuthFlow implements vscode.Disposable {
    ...
    public async getExternalCallbackUri(): Promise<vscode.Uri> {
        return vscode.env.asExternalUri(vscode.Uri.parse(`${vscode.env.uriScheme}://stackbuild.vscode-printstream/did-authenticate`));
    }

    public async login(state: string = uuid.generateUuid(), timeoutSeconds = 60 * 10 /* 10min */, openExternalUrl = true): Promise<string> {
