...
export interface InspectValues<T>
  extends InspectBaseValues<T>,
    InspectLanguageValues<T> {}
...
interface InspectKeyMap
  extends InspectBaseValuesKeyNames,
    InspectLanguageValuesKeyNames {}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
interface InspectBaseToLanguageKeyNames
  extends Record<
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    keyof InspectBaseValues<any>,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    keyof InspectLanguageValues<any>
  > {
...
type InspectKeyNamesTuple = [
  base: keyof InspectBaseValuesKeyNames,
  language: keyof InspectLanguageValuesKeyNames
];
const keyNamesTuple: InspectKeyNamesTuple[] = (
  Object.keys(
    inspectBaseToLanguageKeyNames
  ) as (keyof InspectBaseToLanguageKeyNames)[]
).map((k) => [k, inspectBaseToLanguageKeyNames[k]] as InspectKeyNamesTuple);

type SectionKey = string;
type BaseLanguageSection =
  | ["[*]"]
  | ["[*]", undefined]
  | ["[*]", LanguageKeyId];
  ...
  const cfg: MockWorkspaceConfiguration<T> = {
    get: jest.fn(
      (section, defaultValue) =>
        getValueFromInspect(data, bls, sectionKey(section)) ?? defaultValue
    ) as GetFn,
    has: jest.fn(
      (section) =>
        getValueFromInspect(data, bls, sectionKey(section)) !== undefined
    ),
    inspect: jest.fn((section) =>
      getInspectForPath(data, bls, sectionKey(section))
    ),
    update: jest.fn((section, ...args) =>
      pVoid(updateInspect(data, bls, sectionKey(section), ...args))
    ),
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function getKeyValueFromRecord(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
function updateInspect<T>(
  ...
  const inspectKey = fieldToUpdate(
    bls,
    configurationTarget,
    overrideInLanguage
  );
...
function normalizeConfigurationTarget(
  t?: ConfigurationTarget | boolean | null
): ConfigurationTarget {
...
function scopeToLanguageId(
  scope?: vscode.ConfigurationScope | null
): LanguageKeyId | undefined {
  if (!scope) return undefined;
  if ((<WithLanguageId>scope).languageId)
    return `[${(<WithLanguageId>scope).languageId}]`;
  return undefined;
}
