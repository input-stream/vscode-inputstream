...
export function createMockFileSystem(
  provider: FileSystemProvider = createMockFileSystemProvider()
): FileSystem {
  const fs: FileSystem = {
    stat: jest.fn((...p) => Promise.resolve(provider.stat(...p))),
    readDirectory: jest.fn((...p) =>
      Promise.resolve(provider.readDirectory(...p))
    ),
    readFile: jest.fn((...p) => Promise.resolve(provider.readFile(...p))),
    rename: jest.fn((oldUri, newUri, opt) =>
      Promise.resolve(
        provider.rename(oldUri, newUri, {
          ...opt,
          overwrite: opt?.overwrite ?? false,
        })
      )
    ),
    createDirectory: jest.fn((...p) =>
      Promise.resolve(provider.createDirectory(...p))
    ),
    copy: jest.fn((src, target, opt) =>
      Promise.resolve(
        provider.copy?.(src, target, {
          ...opt,
    ...
    writeFile: jest.fn((...p) =>
      Promise.resolve(
        provider.writeFile(...p, { create: true, overwrite: true })
      )
    ),
    delete: jest.fn((uri, opt) =>
      Promise.resolve(
        provider.delete(uri, { recursive: opt?.recursive ?? true })
      )
    ),
