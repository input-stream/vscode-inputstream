...
export class MockWorkspace implements Workspace {
  ...
  getConfiguration = jest.fn(
    (...args: Parameters<Workspace["getConfiguration"]>) =>
      this.__mockConfig.__getConfiguration(...args)
  );
  getWorkspaceFolder = jest.fn((uri) =>
    getWorkspaceFolder(uri, this.workspaceFolders || [])
  );
...
function openTextDocument(
  param?: string | vscode.Uri | OpenTextDocumentOptions
  ...
  const uri =
    typeof param === "string"
      ...
      : undefined;
  const options =
    typeof param !== "string" && !isUri(param) ? param : undefined;
  if (uri) {
