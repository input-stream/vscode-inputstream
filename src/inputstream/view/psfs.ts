...
export class PsFileExplorer implements vscode.Disposable {
    ...
    protected explorer: InputFileSystemProvider;

    constructor(
        onDidInputChange: vscode.Event<Input>,
    ) {
        this.explorer = new InputFileSystemProvider();
        ...
        onDidInputChange(Input => {
            this.explorer.setInput(Input);
...
export class InputFileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	async getChildren(element?: Entry): Promise<Entry[]> {
		if (element) {
            ...
            files.push({
                uri: vscode.Uri.parse(`psfs:${element.Input.id}/main.md`),
        ...
        const root: Entry = {
            uri: vscode.Uri.parse(`psfs:${this.input.id}/`),
