...
export interface ImageSearchRenderProvider {
    render(opts: RenderingOptions): void
}

/**
 * A webview for search results.
 */
export class ImageSearchPanel {
    ...
    constructor(
        ...
        this.panel = ctx.add(window.createWebviewPanel(id, title, column, {
            enableScripts: true,
        ...
        this.panel.iconPath = {
            dark: mediaIconUri(ctx.extensionUri, MediaIconName.UnsplashDark),
            light: mediaIconUri(ctx.extensionUri, MediaIconName.UnsplashLight),
        };

        ctx.add(this.panel.webview.onDidReceiveMessage(async (message: Message) => {
        ...
        this.onDidChangeHTMLSummary = ctx.add(new event.Emitter<string>());
        this.onDidChangeHTMLResults = ctx.add(new event.Emitter<string>());

        ctx.add(this.onDidChangeHTMLSummary.event(async (html: string) => this.postMessage({
            command: 'innerHTML',
            type: 'div',
            id: 'summary',
            value: html,
        ...
        ctx.add(this.onDidChangeHTMLResults.event(async (html: string) => this.postMessage({
            command: 'innerHTML',
            type: 'div',
            id: 'results',
            value: html,
    ...
    htmlHead(title?: string): string {
        const bootstrapCssUri = cssResourceUri(this.ctx.extensionUri, 'bootstrap.min.css');
        const codeCssUri = cssResourceUri(this.ctx.extensionUri, 'code.css');
